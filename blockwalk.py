...
def update(ims,step):
    ...
    # ただし、上の空隙ブロックに隣接する片方のブロックが空隙なら、その空隙ブロックが存在する方向と逆方向に必ず進む
    elif maze[pacman_position[1]+1, pacman_position[0]-1] == 1 and maze[pacman_position[1]+1, pacman_position[0]+1] == 0:
        move = moves[1]
    elif maze[pacman_position[1]+1, pacman_position[0]-1] == 0 and maze[pacman_position[1]+1, pacman_position[0]+1] == 1:
        move = moves[2]
    else: 
